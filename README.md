...
本项目面向绘本插图数据的全流程处理：批量上传图片 → AI 生成中文描述 → 统一向量化 → 存入数据库与向量库 → 前端支持传统与多维度加权语义搜索（含自动降级与超时容错）→ 智能下载管理与重复过滤。
...
- **📚 智能下载管理**：自动记录下载历史，分类管理，标签系统
- **🚫 重复素材过滤**：搜索时自动过滤已下载图片，避免同质化文案匹配重复素材
...
### 功能特性
- **📥 下载记录管理**：自动记录每次图片下载，包含下载时间、次数统计
- **🏷️ 分类与标签**：支持自定义分类（人物、场景、动物等）和标签系统
- **🔍 智能搜索**：在下载库中按名称、书名、描述进行搜索
- **📊 统计面板**：显示总素材数、近期下载量、分类分布、存储大小
- **✏️ 编辑功能**：可修改图片分类和标签，删除下载记录
- **🎨 多视图模式**：支持网格和列表两种查看模式
- **🚫 自动过滤**：文案匹配时可选择过滤已下载图片，避免重复素材

### 使用方法
1. **启用过滤**：在文图匹配页面勾选"自动过滤已下载图片"
2. **下载图片**：点击匹配结果中的下载按钮，自动记录到下载库
3. **管理下载**：切换到"下载记录"标签页查看和管理已下载的插图
4. **分类整理**：为下载的图片添加分类和标签，便于后续查找

### 数据库初始化
执行以下SQL脚本初始化下载库功能：
```sql
-- 在Supabase控制台执行
\i sql/init_download_library.sql
```

### 清理测试数据
如果发现下载记录中有大量测试数据，可以执行清理脚本：
```sql
-- 查看和清理测试数据
\i sql/cleanup_download_library.sql
```
**注意**：下载记录只有在用户实际点击下载按钮时才会被创建，搜索匹配结果本身不会产生下载记录。
