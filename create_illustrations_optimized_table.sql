-- 创建优化后的 illustrations_optimized 表 SQL DDL
...
-- 为AI描述创建GIN索引（支持LIKE和ILIKE搜索）
-- 注意：由于Supabase可能不支持中文全文搜索配置，我们使用更通用的方法
CREATE INDEX idx_illustrations_optimized_ai_description_gin ON illustrations_optimized USING GIN(ai_description gin_trgm_ops);

-- 如果上面的索引创建失败，可以使用这个简单的B-tree索引
-- CREATE INDEX idx_illustrations_optimized_ai_description_btree ON illustrations_optimized(ai_description);
