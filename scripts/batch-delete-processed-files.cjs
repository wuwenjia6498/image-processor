...
async function deleteDatabaseRecord(id) {
  ...
  if (record?.image_url) {
    try {
      ...
      console.warn(`⚠️ 删除存储文件时出错 (${record.filename}):`, storageError.message);
      // 不抛出错误，因为数据库记录已经删除成功
    }
  }
...
async function batchDeleteProcessedFiles() {
  ...
  for (let i = 0; i < successfulFiles.length; i++) {
    ...
    try {
      console.log(`[${i + 1}/${successfulFiles.length}] 删除: ${filename} (ID: ${id})`);
      
      const result = await deleteDatabaseRecord(id);
  ...
  if (results.errors.length > 0) {
    ...
    results.errors.forEach((error, index) => {
      console.log(`  ${index + 1}. ${error.filename} (${error.id}): ${error.error}`);
    });
  }
  ...
  const report = {
    ...
    results,
    deletedFiles: successfulFiles.filter((_, i) => i < results.success + results.notFound)
  };
